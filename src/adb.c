...
{
	if (db->adb.len < sizeof(adb_val_t)) return ADB_VAL_NULL;
...
{
	if (i >= o->num) return ADB_VAL_NULL;
	return o->obj[i];
}
...
uint32_t adb_ro_int(const struct adb_obj *o, unsigned i)
{
	...
	if (val == ADB_VAL_NULL && o->schema && o->schema->get_default_int)
...
static adb_val_t adb_w_error(struct adb *db)
{
	assert(0);
	db->hdr.magic = 0;
	return ADB_VAL_NULL;
...
adb_val_t adb_w_blob(struct adb *db, apk_blob_t b)
{
	...
	return htole32(o | adb_w_data(db, vec, ARRAY_SIZE(vec), vec[0].iov_len));
}

adb_val_t adb_w_int(struct adb *db, uint32_t val)
{
	if (val >= 0x10000000)
		return htole32(ADB_TYPE_INT_32 | adb_w_data1(db, &val, sizeof val, sizeof val));
	return htole32(ADB_TYPE_INT | val);
}
...
adb_val_t adb_w_copy(struct adb *db, struct adb *srcdb, adb_val_t v)
{
	...
	case ADB_TYPE_ARRAY: {
		...
		sz = adb_ro_num(&obj);
		if (sz > ARRAY_SIZE(cpy)) {
			fprintf(stderr, "%x\n", v);
			return adb_w_error(db);
		}
		cpy[ADBI_NUM_ENTRIES] = obj.obj[ADBI_NUM_ENTRIES];
		for (int i = ADBI_FIRST; i < sz; i++) cpy[i] = adb_w_copy(db, srcdb, adb_ro_val(&obj, i));
		return htole32(ADB_VAL_TYPE(v) | adb_w_data1(db, cpy, sizeof(adb_val_t[sz]), sizeof(adb_val_t)));
	}
...
copy:
	...
	return htole32(ADB_VAL_TYPE(v) | adb_w_data1(db, ptr, sz, align));
}
...
adb_val_t adb_w_adb(struct adb *db, struct adb *valdb)
{
	...
	if (valdb->adb.len <= 4) return ADB_VAL_NULL;
	bsz = htole32(iovec_len(vec, ARRAY_SIZE(vec)) - sizeof bsz);
	return htole32(ADB_TYPE_BLOB_32 | adb_w_raw(db, vec, ARRAY_SIZE(vec), iovec_len(vec, ARRAY_SIZE(vec)), sizeof(uint32_t)));
}
...
static adb_val_t __adb_w_obj(struct adb_obj *o, uint32_t type)
{
	uint32_t max = o->obj[ADBI_NUM_ENTRIES];
	uint32_t n = o->num;
	adb_val_t *obj = o->obj, val = ADB_VAL_NULL;

	if (o->schema && o->schema->pre_commit) o->schema->pre_commit(o);
	while (n > 1 && obj[n-1] == ADB_VAL_NULL) n--;
	if (n > 1) {
		obj[ADBI_NUM_ENTRIES] = htole32(n);
		val = htole32(type | adb_w_data1(o->db, obj, sizeof(adb_val_t[n]), sizeof(adb_val_t)));
	}
	adb_wo_reset(o);
	o->obj[ADBI_NUM_ENTRIES] = max;
	return val;
...
{
	if (i >= o->obj[ADBI_NUM_ENTRIES]) return 0;
...
int adb_wa_append(struct adb_obj *o, adb_val_t v)
{
	...
	if (o->num >= o->obj[ADBI_NUM_ENTRIES]) return 0;
	o->obj[o->num++] = v;
	return 1;
}
